#!/usr/bin/env python3
"""
PyTorch implementation of RNNoise training based on the original Keras code
Directly uses binary data from denoise.c output
"""
...
class RNNoiseModel(nn.Module):
    """
    PyTorch implementation of RNNoise model based on original Keras architecture
    ...
    def __init__(self, l2_reg: float = 1e-6, constraint_clip: float = 0.499):
        ...
        self.constraint_clip = constraint_clip
        self.l2_reg = l2_reg
        
        # Input dense layer: 42 -> 24 with tanh activation
        ...
        for module in self.modules():
            if isinstance(module, (nn.Linear, nn.GRU)):
                if hasattr(module, 'weight') and module.weight is not None:
                    with torch.no_grad():
                        module.weight.clamp_(-self.constraint_clip, self.constraint_clip)
                if hasattr(module, 'bias') and module.bias is not None and not isinstance(module.bias, bool):
                    with torch.no_grad():
                        module.bias.clamp_(-self.constraint_clip, self.constraint_clip)
    
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass matching Keras implementation exactly
        ...
        # Apply ReLU activation to hidden states (matching Keras 'relu' activation)
...
class BinaryDataset(Dataset):
    """Dataset class for binary data from denoise.c output"""
    
    def __init__(self, data_file: str, window_size: int = 2000):
        """
        Initialize dataset with binary data
        
        Args:
            data_file: Path to binary file created by denoise.c
        ...
        # Extract different components (matching Keras data loading)
...
# Loss functions matching Keras implementation exactly

def mymask(y_true: torch.Tensor) -> torch.Tensor:
    """Keras mymask function: K.minimum(y_true+1., 1.)"""
    return torch.minimum(y_true + 1.0, torch.ones_like(y_true))


def my_crossentropy(y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
    """Keras my_crossentropy function: K.mean(2*K.abs(y_true-0.5) * K.binary_crossentropy(y_pred, y_true), axis=-1)"""
...
def msse(y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
    """Keras msse function: K.mean(mymask(y_true) * K.square(K.sqrt(y_pred) - K.sqrt(y_true)), axis=-1)"""
    mask = mymask(y_true)
...
def mycost(y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
    """Keras mycost function: K.mean(mymask(y_true) * (10*K.square(K.square(K.sqrt(y_pred) - K.sqrt(y_true))) + K.square(K.sqrt(y_pred) - K.sqrt(y_true)) + 0.01*K.binary_crossentropy(y_pred, y_true)), axis=-1)"""
    mask = mymask(y_true)
    sqrt_true = torch.sqrt(torch.clamp(y_true, min=1e-8))
    sqrt_pred = torch.sqrt(torch.clamp(y_pred, min=1e-8))
...
def my_accuracy(y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
    """Keras my_accuracy function: K.mean(2*K.abs(y_true-0.5) * K.equal(y_true, K.round(y_pred)), axis=-1)"""
    weight = 2 * torch.abs(y_true - 0.5)
    equal = torch.eq(y_true, torch.round(y_pred)).float()
    # Ensure weight and equal have compatible shapes
    if weight.dim() > equal.dim():
        weight = weight.squeeze(-1)  # Remove last dimension if it's 1
    return torch.mean(weight * equal)
...
def train_epoch(model: nn.Module, 
    ...
    for batch_idx, (x, y_denoise, y_vad) in enumerate(dataloader):
        ...
        # Compute losses (matching Keras loss_weights=[10, 0.5])
        denoise_loss = mycost(y_denoise, pred_denoise)
        vad_loss = my_crossentropy(y_vad, pred_vad)
...
def main():
    parser = argparse.ArgumentParser(description='PyTorch RNNoise Training (Binary Data)')
    parser.add_argument('--data-file', type=str, default='training_10000.f32',
                        help='Path to binary training data file from denoise.c')
    ...
    parser.add_argument('--output-model', type=str, default='weights.pth',
    ...
    # Load dataset
    full_dataset = BinaryDataset(args.data_file, args.window_size)
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, 
                             shuffle=True, num_workers=0)  # num_workers=0 to avoid MPS issues
